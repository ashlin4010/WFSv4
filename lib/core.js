...
if(process.pkg === undefined){
    ...
    definitions = require(path.join(path.dirname(process.pkg.entrypoint),"\\lib\\loader.js")).definitions;
}
...
function realPath(address) {
    ...
    return path.join(Root,safeAddress.join("/"));
}
...
function fileInfo(dir) {
    //for some reason I am get two errors
    let files = fs.readdirSync(dir);
    let output = [];
    const error_case = [
        ['EBUSY', (file)=>{return logger.error("EBUSY: resource busy or locked "+file)}],
        ['EPERM', (file)=>{return logger.error("EPERM: operation not permitted "+file)}]
    ];
    for (let i = 0; i < files.length; i++) {
        try {
            ...
            for (let j = 0; error_case.length > j; j++) {
                if (err.code === error_case[j][0]) error_case[j][1](files[i]);
            }
        }
    }
    return output;
