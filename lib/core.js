...
//const definitions = require("./fileDefinition.js");
...
function fileInfo(dir) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (info.isDirectory()) {
            ...
            //console.log(fileExtension2(files[i]));
            file = {
                name: files[i],
                dateModified: formatDate(info.mtime),
                directory: false,
...
function fileExtension(fileName) {
    ...
    let image = [".JPG", ".tif", ".tiff", ".gif", ".jpeg", ".jpg", ".jif", ".jfif", ".jp2", ".jpx", ".j2k", ".j2c", ".fpx", ".pcd", ".png"];
    let html = [".html"]; //html
...
function fileExtension2(fileName) {
    ...
    for(let i = 0; definitions.length > i; i++){
        if(presentInArray(definitions[i].extname, Extension)){
            return {type: definitions[i].type, icon: definitions[i].icon};
        }
    }
    return {type: "unknown", icon: "icon-doc", action: "?"};
} //Returns a sting that is a class for awesome font
