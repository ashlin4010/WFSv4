...
const services = require("./../privateLib/serviceLoader.js").readAll(service_path);
...
const definitionLoader = require("./../privateLib/fileDefinition.js");
let definitions = {file:[]}; //just in case undefined would be a problem
definitions.file  = definitions.file.concat( definitionLoader.readAll(definitions_path)); //load form definitions_path
definitions.file = definitions.file.concat(generateDefinitionsServices(services)); //load form services
//get special definitions for directorys and unknown
...
function generateRoutes(services) {
    ...
    for (let i = 0; i < services.length; i++) {
        let authentication = auth.pass;
        if (config.server.enableKey && services[i].requireAuthentication) authentication = auth.verify;
        try {
        ...
        router.use("/"+services[i].URLPrecursors, authentication, service);
    }
    return router;
}
